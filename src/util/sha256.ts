// Minimal synchronous SHA-256, used to seed the BC-UR fountain code PRNG.
// WebCrypto is async-only, and the fountain encoder/decoder need it synchronously.
...
export function sha256(data: Uint8Array): Uint8Array {
  ...
  for (let off = 0; off < padded.length; off += 64) {
    ...
    for (let i = 0; i < 8; i++) h[i] = h[i]! + state[i]!;
  }
